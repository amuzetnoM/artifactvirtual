...
# Load .env for system prompt
load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), '.env'))
ENV_PATH = os.path.join(os.path.dirname(__file__), '.env')
with open(ENV_PATH, encoding='utf-8') as f:
    env_content = f.read()
match = re.search(r'<<<<(.*?)>>>>', env_content, re.DOTALL)
SYSTEM_PROMPT = match.group(1).strip() if match else None

# Ensure Ollama is running and models are loaded
try:
    subprocess.Popen(["ollama", "serve"])
    time.sleep(2)  # Give Ollama a moment to start
    subprocess.run(["ollama", "run", "qwen3"], check=False)
    subprocess.run(["ollama", "run", "gemma3"], check=False)
...
class CockpitOrchestrator:
    def __init__(self):
        ...
        # Initialize Gemma3 via Ollama
        try:
            self.llm = Ollama(model="gemma3")
        except Exception as e:
            print(f"[LLM Init Error] {e}")
            self.llm = None
        # Initialize Qwen3 via Ollama for instruction generation
        try:
            self.qwen = Ollama(model="qwen3")
        except Exception as e:
            print(f"[Qwen3 LLM Init Error] {e}")
            self.qwen = None

    def _call_gemma3(self, prompt):
        # Use system prompt as context for Gemma3
        if self.llm:
            try:
                ...
                return prompt
        # Fallback: regex parser if LLM is not available
    ...
    def _call_qwen3(self, prompt):
        # Use system prompt as context for Qwen3
        if self.qwen:
            try:
                full_prompt = f"[SYSTEM PROMPT]\n{self.system_prompt}\n[USER PROMPT]\n{prompt}"
                response = self.qwen.invoke(full_prompt)
                return response
            except Exception as e:
    ...
    def _init_pgvector(self):
        # Disabled for now: advanced persistent memory integration
        return None
    ...
    def chat_qwen(self):
        ...
        while True:
            user_prompt = input("[Qwen3 User] > ")
            if user_prompt.strip().lower() == 'exit':
                print("[Qwen3] Session ended.")
                break
            self.chat_history.append({"role": "user", "content": user_prompt})
            # Qwen3 LLM for instruction generation
            instruction = self._call_qwen3(user_prompt)
            print(f"[Qwen3 Output] {instruction}")
            # Try to map and execute the instruction
            cli_command = self._map_to_cli(instruction)
            if cli_command:
                cli_tool = self.tool_registry.get_tool('cli')
                result = cli_tool.execute(cli_command)
                print(f"[CLI Result] {result}")
            else:
                print(f"[Qwen3] No CLI mapping for: {instruction}")
            self.chat_history.append({"role": "system", "content": str(instruction)})
